...
namespace ShrineFoxCom.Resources.Browse
{
    public class Webscraper
    {
        ...
        public static void UpdateTSVs(PlaceHolder control)
        {
            ...
            {
                foreach (var game in platform.Games.Where(x => x.GBURL != ""))
                {
                    ...
                    foreach (string type in Enum.GetNames(typeof(TypeFilter)))
                    {
                        ...
                        for (int i = 1; i < 10; i++)
                        {
                            if (!noneFound)
                            {
                                Task.Run(async () =>
                                {
                                    await FeedGenerator.GetFeed(i, game.GBURL, (TypeFilter)Enum.Parse(typeof(TypeFilter), type));
                                        ...
                                        if (!Posts.Any(y => y.URL.TrimEnd('/').EndsWith("/" + item.Link.ToString().TrimEnd('/').Split('/').Last())))
                                        {
                                            if (!exclusions.Any(x => item.Title.ToLower().Contains(x.ToLower()) || item.Owner.Name.ToLower().Equals(x.ToLower())))
                                            {
                                                ...
                                                if (post.URL.Contains("/tuts/"))
                                                    ...
                                                    post.Type = "mod";
                                                post.Games = new List<string>() { game.SFName };
                                                post.Tags = new List<string>() { item.CategoryName.Replace("Other/", "").Replace("Game file", "").Trim(' ') };
                                                post.Title = item.Title;
                                                post.SourceURL = "";
                                                Regex rgx = new Regex("[^a-zA-Z0-9 -]");
                                                post.Id = game.SFName.ToLower() + "-" + rgx.Replace(item.Title.ToLower().Replace(" ", ""), "");
    ...
    public static class FeedGenerator
    {
        ...
        public static async Task GetFeed(int page, string gameSectionId, TypeFilter type)
        {
            error = false;
            if (feed == null)
            ...
            {
                var requestUrl = GenerateUrl(page, gameSectionId, type);
        ...
        private static string GenerateUrl(int page, string gameSectionId, TypeFilter type)
        {
            // Base
            ...
            url += $"ByGame?_aGameRowIds[]={gameSectionId}";
            // Consistent args
